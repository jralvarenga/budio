...
export async function getUserTransactions(): Promise<Transaction[]> {
  ...
  const { data: transaction, error } = await supabase.from("Transaction").select("*").eq('user_id', user!.id).returns<Transaction[]>()
...
export async function createTransaction(transaction: Partial<Transaction>): Promise<Transaction> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  const { data, error } = await supabase.from("Transaction").upsert({
    ...
    user_id: user!.id,
  } as Partial<Transaction>).select('*').returns<Transaction[]>()
